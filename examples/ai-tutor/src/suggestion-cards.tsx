...
/**
 * A question offered as a card with real footage on it.
 *
 * Used twice, and deliberately the same both times: before the session starts,
 * where four cards are the whole invitation, and after an answer finishes,
 ...
 * like different kinds of thing when they are the same thing - a question the
 * tutor is willing to answer next.
 *
 ...
 * just wears the ground colour, which is the state the rest of the tutor
 * already handles.
 */
...
export function SuggestionCards({ suggestions, label, onAsk }: {
  ...
  return <>
    <ul className="cards" ref={railRef} aria-label={label}>
      {suggestions.map((card, index) => <li key={card.question}>
        <button
          type="button"

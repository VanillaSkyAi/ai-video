...
import { getBackgroundTransform } from "../src/visual-system/scene-templates/background-effect";
...
describe("default media scene-boundary continuity", () => {
  it("has no implicit transform to reset at the scene boundary", () => {
    expect(getBackgroundTransform(undefined, 0).transform).toBe("none");
    expect(getBackgroundTransform(undefined, 1).transform).toBe("none");
  });
});

...
function sseResponse(requestId: string, value: string): Response {
  ...
  const events = [
    { protocolVersion: "0.5", type: "response.start", eventId: "run:0", runId: "run", sequence: 0, data: { requestId, format: { orientation: "portrait" }, style: TEST_VIDEO_STYLE, capabilities: { templates: ["metric"] } } },
    { protocolVersion: "0.5", type: "scene.add", eventId: "run:1", runId: "run", sequence: 1, data: { scene: { id: "one", templateId: "metric", variables: { value }, timing: { fixedDuration: 4, startTime: 0, endTime: 4 } }, position: 0 } },
    { protocolVersion: "0.5", type: "response.complete", eventId: "run:2", runId: "run", sequence: 2, data: { finishReason: "stop", snapshot, checksum: checksumVideo(snapshot) } },
  ];
...
function terminalErrorResponse(requestId: string): Response {
  const events = [
    { protocolVersion: "0.5", type: "response.start", eventId: "failed-run:0", runId: "failed-run", sequence: 0, data: { requestId, format: { orientation: "portrait" }, style: TEST_VIDEO_STYLE, capabilities: { templates: ["metric"] } } },
    { protocolVersion: "0.5", type: "response.error", eventId: "failed-run:1", runId: "failed-run", sequence: 1, data: { error: { code: "generation_failed", message: "Video response generation failed", recoverable: false }, terminal: true, snapshot: { schemaVersion: "0.1", orientation: "portrait", scenes: [], style: TEST_VIDEO_STYLE } } },
  ];
  return new Response(events.map((event) => `data: ${JSON.stringify(event)}\n\n`).join("") + "data: [DONE]\n\n", {
    status: 200,
    headers: { "content-type": "text/event-stream", "x-vanillasky-video-stream": "0.5" },
...
function terminalAbortResponse(requestId: string): Response {
  ...
  const events = [
    { protocolVersion: "0.5", type: "response.start", eventId: "aborted-run:0", runId: "aborted-run", sequence: 0, data: { requestId, format: { orientation: "portrait" }, style: TEST_VIDEO_STYLE, capabilities: { templates: ["metric"] } } },
    { protocolVersion: "0.5", type: "response.abort", eventId: "aborted-run:1", runId: "aborted-run", sequence: 1, data: { reason: "server stopped", snapshot } },
  ];
  return new Response(events.map((event) => `data: ${JSON.stringify(event)}\n\n`).join("") + "data: [DONE]\n\n", {
    status: 200,
    headers: { "content-type": "text/event-stream", "x-vanillasky-video-stream": "0.5" },
...
function warningResponse(requestId: string): Response {
  ...
  const events = [
    { protocolVersion: "0.5", type: "response.start", eventId: "warning-run:0", runId: "warning-run", sequence: 0, data: { requestId, format: { orientation: "portrait" }, style: TEST_VIDEO_STYLE, capabilities: { templates: ["metric"] } } },
    { protocolVersion: "0.5", type: "response.warning", eventId: "warning-run:1", runId: "warning-run", sequence: 1, data: { warning: { code: "provider_warning", category: "provider", message: "The model provider reported a warning.", recoverable: true } } },
    { protocolVersion: "0.5", type: "scene.add", eventId: "warning-run:2", runId: "warning-run", sequence: 2, data: { scene: snapshot.scenes[0], position: 0 } },
    { protocolVersion: "0.5", type: "response.complete", eventId: "warning-run:3", runId: "warning-run", sequence: 3, data: { finishReason: "stop", snapshot, checksum: checksumVideo(snapshot) } },
  ];
  return new Response(events.map((event) => `data: ${JSON.stringify(event)}\n\n`).join("") + "data: [DONE]\n\n", {
    headers: { "content-type": "text/event-stream", "x-vanillasky-video-stream": "0.5" },
  });
...
describe("useVideo", () => {
  ...
  it("defaults to /api/video and returns the completed video", async () => {
    ...
    expect(result.current.playerProps.templates).toBe(kit);
  ...
  it("reports an incompatible server protocol version distinctly from a missing header", async () => {
    ...
    await expect(generation).rejects.toThrow(
      "Video response protocol mismatch: expected 0.5, received 0.2",
    );
  });
  ...
  it("uses the trusted built-in templates when no registry is configured", async () => {
    ...
    expect(result.current.playerProps).not.toHaveProperty("templates");
  });

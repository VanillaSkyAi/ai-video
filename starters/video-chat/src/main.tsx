...
import { VideoPlayer, useVideoChat, type VideoChatMode } from "@vanillaskyai/video/react";
...
function App() {
  ...
  const [modeId, setModeId] = useState<VideoChatMode>(defaultMode.id);
  ...
  const chat = useVideoChat({
    ...
    brand: theme.brand,
    style: { generatedLook: theme.generatedLook },
  });
  ...
  const stageOrientation = shown?.fixedOrientation ? shownOrientation : viewportOrientation;
    ...
    <div className="stage-area">
      <div className="stage" style={{ background: themeBackground(theme) }}>
        {!showing && <>
          ...
          {chat.turns.length === 0 && <Welcome data={chat.welcome} onAsk={ask} />}
      ...
      >
        ...
        <fieldset>
          ...
          {availableModes.map((option) => <label key={option.id}>
            <input type="radio" name="visuals" checked={option.id === selectedMode.id} onChange={() => setModeId(option.id)} />
